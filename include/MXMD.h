...
public:
	...
	virtual MXMDVertexBuffer::DescriptorCollection GetDeltaMorph(int id) const = 0;
...
public:
	...
	virtual MXMDMorphTargets::Ptr GetVertexBufferMorphTargets(int vertexBufferID) const = 0;
...
{
	template<class _Ty>
	int _ExtractAllTextures(const _Ty *outputFolder, TextureConversionParams params) const;
public:
	...
	int ExtractAllTextures(const wchar_t *outputFolder, TextureConversionParams params) const { return _ExtractAllTextures(outputFolder, params); }
	int ExtractAllTextures(const char *outputFolder, TextureConversionParams params) const { return _ExtractAllTextures(outputFolder, params); }
...
class MXMD
{
	static const int ID_BIG = CompileFourCC("MXMD");
	static const int ID = CompileFourCC("DMXM");
	union
	{
		char *masterBuffer;
		MXMDHeader *header;
