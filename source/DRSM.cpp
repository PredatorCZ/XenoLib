...
template<class _Ty0>
int DRSM::_ExtractTexture(const _Ty0 *outputFolder, int id, TextureConversionParams params) const
{
	DRSMTextureItem *cTex = data.header->TextureTable()->Textures() + id;
	...
	const UniString<_Ty0> outputName = outputFolder + esStringConvert<_Ty0>(textureName);
	char *hrMipBuffer = nullptr,
		*midMipBuffer = nullptr;
	int hrMipBufferSize = 0,
		midMipBufferSize = 0;
	...
	if (ConvertLBIM(midMipBuffer, midMipBufferSize, outputName.c_str(), params, hrMipBuffer, hrMipBufferSize))
	{
...
template int DRSM::_ExtractTexture(const char *outputFolder, int id, TextureConversionParams params) const;
template int DRSM::_ExtractTexture(const wchar_t *outputFolder, int id, TextureConversionParams params) const;
...
char *ExtractXBC(const char *buffer)
{
	...
	Bytef *comBuffer = const_cast<Bytef *>(reinterpret_cast<const Bytef *>(hdr + 1));
