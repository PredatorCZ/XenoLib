...
void WritePng(std::ofstream *stream, const char *buffer, int /*size*/,
              int width, int height, int colorType, int bpr, bool flipRB) {
  png_structp pngStruct = nullptr;
  png_infop pngInfo = nullptr;
  png_voidp user_error_ptr = nullptr;

  pngStruct = png_create_write_struct(PNG_LIBPNG_VER_STRING, user_error_ptr,
                                      _pngerrorfunc, _pngwarningfunc);
  ...
  for (int r = 0; r < height; r++)
    png_write_row(pngStruct, reinterpret_cast<const unsigned char *>(
                                 buffer + (r * width * bpr)));
