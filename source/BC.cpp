...
void BCHeader::Fixup()
{
	...
	for (int f = 0; f < numPointers; f++)
		*reinterpret_cast<esIntPtr *>(thisAddr + fixupsOffset[f]) += reinterpret_cast<esIntPtr>(thisAddr);

	numPointers = 0;
}

template<class _Ty0>
int BC::_Load(const _Ty0 *fileName, bool suppressErrors)
{
...
{
	out = (delta * delta * delta) * items[0] + (delta * delta) * items[1] + delta * items[2] + items[3];
}

ES_INLINE void BCANIM::CubicVector3Frame::Evaluate(Vector &out, float delta) const
{
...
ES_INLINE void BCANIM::CubicQuatFrame::Evaluate(Vector4 &out, float delta) const
{
	elements[0].Evaluate(out[0], delta);
	elements[1].Evaluate(out[1], delta);
	elements[2].Evaluate(out[2], delta);
...
ES_INLINE void GetEvalValue(const T &frames, float time, BCANIM *hdr, V &out)
{
	...
	if (frames.count > 1)
	{
		...
		if (lastFrame > frames.data.ptr + frames.count - 1)
			lastFrame = frames.data.ptr + frames.count - 1;
...
ES_INLINE void BCANIM::AnimationTrack::GetPosition(float time, Vector &out, BCANIM *hdr) const
{
	GetEvalValue(position, time, hdr, out);
}

ES_INLINE void BCANIM::AnimationTrack::GetRotation(float time, Vector4 &out, BCANIM *hdr) const
{
	GetEvalValue(rotation, time, hdr, out);
}

ES_INLINE void BCANIM::AnimationTrack::GetScale(float time, Vector &out, BCANIM *hdr) const
{
	GetEvalValue(scale, time, hdr, out);
}

void BCANIM::AnimationTrack::GetTransform(float time, TransformFrame &out, BCANIM *hdr) const
{
