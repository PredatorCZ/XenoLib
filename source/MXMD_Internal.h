...
template <class E, class C> void _ArraySwap(C &input) {
...
struct MXMDHeader {
  int magic, version, modelsOffset, materialsOffset, unkOffset0,
      vertexBufferOffset, shadersOffset, cachedTexturesOffset, unkOffset1,
      uncachedTexturesOffset,
      // custom values
      externalBufferIDsOffset, externalBufferIDsCount, instancesOffset,
      externalTexturesOffset, externalTexturesCount, null00[2];

  void SwapEndian() { _ArraySwap<int>(*this); }
...
struct MXMDVertexType {
  ...
  void SwapEndian() { _ArraySwap<short>(*this); }
